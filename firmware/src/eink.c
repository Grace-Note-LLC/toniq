...
#define MAX_DISPLAY_BUFFER_SIZE 5000 // Buffer size for 200x200 display
#define MAX_HEIGHT(EPD) (EPD::HEIGHT <= MAX_DISPLAY_BUFFER_SIZE / (EPD::WIDTH / 8) ? EPD::HEIGHT : MAX_DISPLAY_BUFFER_SIZE / (EPD::WIDTH / 8))

// Use the pins defined in main.c
...
void eink_init(void) {
    display.init(115200, true, 2, false); // Initialize with 2ms reset pulse
...
void eink_update_full(const uint8_t* image_data) {
    ...
    do {
        ...
        display.drawBitmap(0, 0, image_data, display.width(), display.height(), GxEPD_BLACK);
...
void eink_update_partial(int16_t x, int16_t y, int16_t w, int16_t h, const uint8_t* image_data) {
    ...
    do {
        display.fillScreen(GxEPD_WHITE);
        display.drawBitmap(x, y, image_data, w, h, GxEPD_BLACK);
    } while (display.nextPage());
}
